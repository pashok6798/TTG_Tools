using System;
using TTG_Tools.DDSReader.Utils;

namespace TTG_Tools.DDSReader
{
	internal class Decompressor
	{
		internal static byte[] Expand(DDSStruct header, byte[] data, PixelFormat pixelFormat)
		{
			...
			switch (pixelFormat)
			{
				...
				case PixelFormat.BC4U:
					rawData = DecompressBC4U(header, data, pixelFormat);
					break;
		...
		private static unsafe byte[] DecompressBC4U(DDSStruct header, byte[] data, PixelFormat pixelFormat)
		{
			// allocate bitmap
			int bpp = (int)(Helper.PixelFormatToBpp(pixelFormat, header.pixelformat.rgbbitcount));
			int bps = (int)(header.width * bpp * Helper.PixelFormatToBpc(pixelFormat));
					...
					for (int y = 0; y < height; y += 4)
					{
						for (int x = 0; x < width; x += 4)
						{
							...
							if (colour0 > colour1)
							{
								...
								// These 2-bit codes correspond to the 2-bit fields
								// stored in the 64-bit block.
								colours[2].blue = (byte)((2 * colours[0].blue + colours[1].blue + 7) / 3);
								colours[2].green = (byte)((2 * colours[0].green + colours[1].green + 7) / 3);
								colours[2].red = (byte)((2 * colours[0].red + colours[1].red + 7) / 3);
								//colours[2].alpha = 0xFF;
							...
							else
							{
								...
								colours[3].blue = (byte)((colours[0].blue + 7 * colours[1].blue) / 3);
								colours[3].green = (byte)((colours[0].green + 7 * colours[1].green) / 3);
								colours[3].red = (byte)((colours[0].red + 7 * colours[1].red) / 3);
								colours[3].alpha = 0x00;
							}
